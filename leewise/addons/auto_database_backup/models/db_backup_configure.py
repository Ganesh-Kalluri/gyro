...
from leewise.tools import find_pg_tool, exec_pg_environ
from leewise.http import request
from leewise.service import db
...
class DbBackupConfigure(models.Model):
    ...
    def _check_db_credentials(self):
        ...
        try:
            leewise.service.db.check_super(self.master_pwd)
        except Exception:
    ...
    def dump_data(self, db_name, stream, backup_format):
        ...
        if backup_format == 'zip':
            with tempfile.TemporaryDirectory() as dump_dir:
                filestore = leewise.tools.config.filestore(db_name)
                ...
                with open(os.path.join(dump_dir, 'manifest.json'), 'w') as fh:
                    db = leewise.sql_db.db_connect(db_name)
                    with db.cursor() as cr:
                        json.dump(self._dump_db_manifest(cr), fh, indent=4)
                if stream:
                    leewise.tools.osutil.zip_dir(dump_dir, stream,
                                              include_dir=False,
                                              fnct_sort=lambda
                                                  file_name: file_name != 'dump.sql')
                else:
                    t = tempfile.TemporaryFile()
                    leewise.tools.osutil.zip_dir(dump_dir, t, include_dir=False,
                                              fnct_sort=lambda
                                                  file_name: file_name != 'dump.sql')
                    t.seek(0)
                    return t
    ...
    def _dump_db_manifest(self, cr):
        ...
        manifest = {
            'leewise_dump': '1',
            'db_name': cr.dbname,
            'version': leewise.release.version,
            'version_info': leewise.release.version_info,
            'major_version': leewise.release.major_version,
